...
export function workspaceWriteSid(workspaceRoot: string): string {
  return workspaceWriteSidForRoots([workspaceRoot])
}

/** Derive one capability identity for an ordered set of workspace roots. */
export function workspaceWriteSidForRoots(workspaceRoots: readonly string[]): string {
  const identity = [...workspaceRoots].join('\0')
