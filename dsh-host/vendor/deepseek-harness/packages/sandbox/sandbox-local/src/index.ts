...
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSidForRoots } from '@deepseek-ai/dsh-sandbox-windows-acl'
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerArgv(policy: SandboxPolicy): string[] {
    const workspaceRoots = [...new Set(policy.workspaceRoots ?? [policy.workspaceRoot])]
    const sessionId = policy.sessionId
    if (sessionId === undefined || policy.mode === 'read-only') {
      return [
        ...this.windowsAclRunnerInvocation(),
        ...workspaceRoots.flatMap(root => ['--workspace', root]),
        '--temp', tmpdir(),
        '--mode', policy.mode,
      ]
    }
    const temp = this.materializeAclGrant(sessionId, workspaceRoots)
    return [
      ...this.windowsAclRunnerInvocation(),
      ...workspaceRoots.flatMap(root => ['--workspace', root]),
      '--temp', temp.dir,
      '--mode', policy.mode,
      '--write-sid', workspaceWriteSidForRoots(workspaceRoots),
  ...
  private materializeAclGrant(sessionId: SessionId, workspaceRoots: readonly string[]): AclTempCapability {
    ...
    const writeSid = workspaceWriteSidForRoots(workspaceRoots)
    const workspaceKey = JSON.stringify(workspaceRoots)
