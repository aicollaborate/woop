...
function resolveWorkspaceRoots(paths: readonly string[], primary: string): string[] {
  return [...new Set([primary, ...paths].map(resolveWorkspaceRoot))]
}
...
function renderPolicyContext(policy: SandboxExecutionPolicy): string {
  switch (policy.mode) {
    ...
    case 'workspace-write':
      return `Current DSH file policy: workspace-write. Any available operation enforced by the DSH file sandbox may modify files under these session workspaces: ${policy.workspaceRoots?.map(root => JSON.stringify(root)).join(', ') ?? JSON.stringify(policy.workspaceRoot)}. Some platform temporary areas may also be writable.`
...
export interface Config {
  ...
  /** Additional Flowix-selected roots that share workspace-write authority. */
...
export class SandboxPolicyService extends Service {
  ...
  constructor(ctx: Context, config: Config) {
    ...
    this.workspaceRoots = resolveWorkspaceRoots(config.workspaceRoots ?? [], this.workspaceRoot)
  ...
  resolve(request: SandboxPolicyRequest = {}): SandboxExecutionPolicy {
    ...
    return {
      ...
      workspaceRoots: resolveWorkspaceRoots(this.workspaceRoots, session?.header.cwd ?? this.workspaceRoot),
