...
构建器还提供两个可选的部署控制项，且不会改变 Python 运行时的默认行为。`--keep-packages` 接受通用部署闭包中的包根，并保留其传递依赖、可选依赖与非可选对等依赖；缺失根包时构建会失败，不会生成不完整的可执行文件。`--launcher` 将一个 CommonJS 宿主复制到暂存目录，使可执行文件默认分派到该宿主，并在 `FLOWIX_DSH_RUNTIME_MODE=1` 时分派到已打包的 JSON-RPC 运行时，因此嵌入方可让两个进程角色共用一个 Node 载体。
