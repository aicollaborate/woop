...
try {
  ...
  check('initialize via /rpc', init.result?.serverInfo?.name === 'dsh-appserver', init.result?.serverInfo)
  ...
  check('second client initializes independently', bInit.result?.serverInfo?.name === 'dsh-appserver')
  ...
  check('initialize with optOut capabilities', optOutInit.result?.serverInfo?.name === 'dsh-appserver')
