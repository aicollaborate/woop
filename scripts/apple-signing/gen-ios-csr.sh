...
# for an Apple Distribution certificate (iOS / iPadOS App Store + TestFlight).
#
...
#
# Apple Personal Developer accounts historically name this cert type "iOS
# Distribution" rather than "Apple Distribution"; both produce the same RSA
# identity for signing .ipa. Pick whatever name your portal offers.
#
# Usage:
#   bash scripts/apple-signing/gen-ios-csr.sh <apple-id-email> <common-name> <org-name> [country-code]
#
# Args:
...
#                     EXACTLY — case and spacing. Personal accounts have
#                     no Org; pass your full legal name here (Apple accepts
#                     the personal name in O= for individual accounts).
#                     Find it at: developer.apple.com → Membership Details
#   country-code      ISO 3166-1 alpha-2 (default: US)
#
# Outputs:
#   ~/.flowix-signing/devid-ios.key   private key (NEVER SHARE, NEVER COMMIT)
#   ~/.flowix-signing/devid-ios.csr   CSR to upload to Apple
#
# After running:
#   1. Open https://developer.apple.com/account/resources/certificates/list
#   2. Click "+" → "Apple Distribution" (or "iOS Distribution" on Personal
#      accounts that don't show "Apple Distribution") → Continue
#   3. Upload devid-ios.csr → Continue → Download the issued .cer
#   4. Run: bash scripts/apple-signing/make-ios-p12.sh path/to/apple_distribution.cer
...
usage: $0 <apple-id-email> <common-name> <org-name> [country-code]
  ...
  org-name         Must EXACTLY match Apple Developer Account's Org name
                   (Membership Details → Organization). Personal accounts
                   use the personal legal name here.
  country-code     ISO 3166-1 alpha-2 (default: US).
EOF
  exit 2
fi
...
if [[ ! "$COUNTRY" =~ ^[A-Z]{2}$ ]]; then
  echo "ERROR: country must be a 2-letter ISO code, got: $COUNTRY" >&2
  exit 2
fi
...
Next steps (these you do in your browser, no credentials go through chat):
  ...
  2. Click "+" → "Apple Distribution" (Personal accounts may see "iOS
     Distribution" instead — both produce the same RSA identity) → Continue

  3. Click "Choose File" → select:
     $CSR

  4. Apple issues a .cer within ~30 seconds. Click "Download" and save the
     .cer somewhere (e.g. ~/Desktop/apple_distribution.cer).

  5. Then run:
     bash $0/../make-ios-p12.sh ~/Desktop/apple_distribution.cer
EOF
