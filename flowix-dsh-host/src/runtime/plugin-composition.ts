...
export function pluginKey(scope: HarnessPluginScope, index: number, id: string, preset?: string): string {
  ...
  return scope === 'host'
    ? `host:${id}`
    : `preset:${preset ?? 'standard'}:${id}`
}

/** Previous releases persisted position-based keys. Read them during the
 * migration window, but never emit new ones: upstream preset reordering must
 * not silently move a user's disable override to another plugin. */
function legacyPluginKey(scope: HarnessPluginScope, index: number, id: string, preset?: string): string {
  return scope === 'host'
    ? `host:${index}:${id}`
...
export function applyPluginDisables(
  ...
  const flush = () => {
    if (current !== undefined && current.name !== undefined && current.name !== 'cordis:group') {
      ...
      const legacyKey = legacyPluginKey(scope, current.index, current.id, preset)
      if ((disabled.has(key) || disabled.has(legacyKey)) && !current.hasDisabled) {
