...
impl super::transport::DshClient for AppServerClient {
    fn next_request_id(&self) -> u64 { self.next_request_id() }
    fn is_closed(&self) -> bool { self.is_closed() }
    async fn request(&self, value: Value) -> Result<Value, String> { self.request(value).await }
    async fn subscribe(&self, thread_id: &str, _run_id: &str) -> mpsc::UnboundedReceiver<Value> { self.subscribe(thread_id).await }
    async fn unsubscribe(&self, thread_id: &str, _run_id: &str) { self.unsubscribe(thread_id).await }
    async fn shutdown(&self) { self.shutdown().await }
}
...
impl AppServerClient {
    pub(crate) async fn spawn(
        ...
        let stdout = child.stdout.take().ok_or("dsh-appserver stdout unavailable")?;
        let stderr = child.stderr.take().ok_or("dsh-appserver stderr unavailable")?;
    ...
    pub(crate) fn next_request_id(&self) -> u64 { self.next_id.fetch_add(1, Ordering::Relaxed) }
    ...
    pub(crate) async fn request(&self, value: Value) -> Result<Value, String> {
        if self.is_closed() { return Err("dsh-appserver is not running".into()); }
        let id = value.get("id").and_then(Value::as_u64).ok_or("App Server request has no numeric id")?;
        let method = value.get("method").and_then(Value::as_str).unwrap_or("unknown");
        ...
        match tokio::time::timeout(std::time::Duration::from_secs(125), rx).await {
            ...
            Err(_) => { self.pending.lock().await.remove(&id); Err("dsh-appserver request timed out".into()) }
        }
    }
    ...
    pub(crate) async fn unsubscribe(&self, thread_id: &str) { self.routes.lock().await.remove(thread_id); }
            ...
            loop {
                ...
                if let Some(id) = message.get("id").and_then(Value::as_u64) {
                    ...
                    if let Some(sender) = client.pending.lock().await.remove(&id) {
                        let result = if let Some(error) = message.get("error") { Err(error.get("message").and_then(Value::as_str).unwrap_or("App Server request failed").to_string()) } else { Ok(message.get("result").cloned().unwrap_or(Value::Null)) };
    ...
    async fn fail_all(&self, message: String) {
        self.closed.store(true, Ordering::Release);
        for (_, sender) in self.pending.lock().await.drain() { let _ = sender.send(Err(message.clone())); }
