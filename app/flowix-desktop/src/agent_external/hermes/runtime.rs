...
use super::acp_command::build_hermes_acp_command;
use super::protocol;
...
#[derive(Default)]
struct HermesTurnEvents {
    events: Vec<CompactedHermesEvent>,
    assistant_index: Option<usize>,
    reasoning_index: Option<usize>,
    next_id: usize,
}

struct CompactedHermesEvent {
    chunk: AgentChunk,
    metadata: AgentChunkMetadata,
}

impl HermesTurnEvents {
    fn observe(&mut self, chunk: &AgentChunk, run_id: &str) {
        match chunk {
            ...
            AgentChunk::ToolCall {
                ...
                self.events.push(CompactedHermesEvent {
                    chunk: chunk.clone(),
    ...
    fn finish(mut self) -> Vec<CompactedHermesEvent> {
        self.close_streaming_rows();
        self.events
    }
    ...
    fn push_text_event(
        ...
        self.events.push(CompactedHermesEvent {
            chunk,
            metadata: AgentChunkMetadata {
                message_id: Some(format!("hermes-{run_id}-{role}-{}", self.next_id)),
...
impl ExternalLifecycleEmitter for HermesAcpManager {
    ...
    async fn persist_emitted_stream_end(&self, _chunk: &AgentChunk, _run_id: &str) {
        // Hermes owns the durable transcript; Flowix only consumes ACP
        // history when the conversation is reopened.
    }
}

impl HermesAcpManager {
    pub fn new(thread_manager: Arc<ThreadManager>) -> Self {
        Self {
            thread_manager,
            runs: ExternalRunRegistry::new(AGENT_TYPE, "Hermes ACP"),
    ...
    pub async fn delete_thread(&self, thread_id: &str) -> Result<bool, String> {
        ...
        crate::agent_external::acp_lifecycle::delete_session(
            build_hermes_acp_command(&cwd, Some("read-only"), None),
            &session_id,
        )
        .await
    }
    ...
    pub async fn supported_models(&self) -> Result<Vec<String>, String> {
        ...
        // Hermes ACP does not expose a separate model catalog endpoint. Keep
        // the same optional catalog contract as OpenCode without probing the
        // interactive CLI (which would create a second Hermes process).
        let models = Vec::new();
    ...
    pub async fn stop_chat(
        ...
        let mut stopped = self
            .runs
            .stop_run(thread_id, thread_id, run_id, "Hermes ACP")
            .await;
        if stopped.is_none() {
            if let Some(mapped) = mapped_thread_id.as_deref() {
                stopped = self
                    .runs
                    .stop_run(mapped, thread_id, run_id, "Hermes ACP")
                    .await;
            }
        }
    ...
    pub async fn stop_all(&self) -> usize {
        ...
        let (count, _finalized) = self
            .runs
            .kill_all_finalized("Hermes ACP", APP_EXIT_REASON)
            .await;
        count
    }
    ...
    async fn run_acp(
        ...
        let mut turn_events = HermesTurnEvents::default();
        let mut child =
            build_hermes_acp_command(&cwd, permission_mode.as_deref(), model.as_deref())
                .stdin(Stdio::piped())
                .stdout(Stdio::piped())
                .stderr(Stdio::piped())
                .spawn()
                .map_err(|error| format!("failed to start Hermes ACP: {error}"))?;
        let child_pid = child.id();
        let stdin =
            Arc::new(Mutex::new(child.stdin.take().ok_or_else(|| {
                "failed to capture Hermes ACP stdin".to_string()
            })?));
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| "failed to capture Hermes ACP stdout".to_string())?;
        let stderr = child
            .stderr
            .take()
            .ok_or_else(|| "failed to capture Hermes ACP stderr".to_string())?;
            ...
            .await
        {
            ...
            return Err("Hermes ACP is already running for this thread".to_string());
        }
        ...
        let protocol_result = async {
            ...
            if negotiated_protocol != Some(protocol::PROTOCOL_VERSION) {
                return Err(format!(
                    "Hermes ACP negotiated unsupported protocol version: {}",
                    negotiated_protocol
            ...
            let session_id = protocol::session_id_from_result(&session_result)
                ...
                .ok_or_else(|| "Hermes ACP did not return a session id".to_string())?;
        ...
        if !stderr_text.trim().is_empty() {
            runtime_log::record_agent_event(
                ...
                "hermes_acp",
                "hermes.acp_stderr",
                "Hermes ACP wrote diagnostic output",
                Some(thread_id),
                Some(AGENT_TYPE),
                Some(serde_json::json!({
        ...
        match protocol_result {
            ...
            Err(error) => {
                ...
                if stderr.is_empty() || error.contains(stderr) {
                    ...
                    // ACP responses are authoritative, but Hermes may put
    ...
    async fn read_until_response(
        ...
        turn_events: &mut HermesTurnEvents,
    ) -> Result<Value, String> {
        loop {
            let Some((line, truncated)) = read_capped_line(stdout, MAX_STDOUT_LINE_BYTES).await?
            else {
                return Err(format!(
                    "Hermes ACP closed before responding to request {response_id}"
                ));
            };
            self.runs.touch(thread_id, Some(run_id)).await;
            if truncated {
                return Err("Hermes ACP emitted an oversized JSON-RPC message".to_string());
            }
            let line = line.trim();
            if line.is_empty() {
                continue;
            ...
            let value: Value = serde_json::from_str(line)
                .map_err(|error| format!("invalid Hermes ACP JSON-RPC: {error}"))?;
...
async fn write_message(stdin: &Arc<Mutex<ChildStdin>>, message: &Value) -> Result<(), String> {
    ...
    stdin
        ...
        .await
        .map_err(|error| format!("failed to write Hermes ACP message: {error}"))?;
    stdin
        .flush()
        .await
        .map_err(|error| format!("failed to flush Hermes ACP message: {error}"))
}
...
mod tests {
    ...
    fn turn_events_compact_stream_chunks_into_snapshot_rows() {
        let mut turn = HermesTurnEvents::default();
        let chunks = [
            AgentChunk::Reasoning {
                thread_id: "thread".into(),
                text: "inspect ".into(),
