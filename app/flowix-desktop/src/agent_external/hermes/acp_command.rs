...
use super::cli::resolve_hermes_binary;
use crate::agent_external::shared::configure_unix_process_group;

pub fn build_hermes_acp_command(
    cwd: &Path,
    permission_mode: Option<&str>,
    model: Option<&str>,
) -> Command {
    let mut command = Command::new(resolve_hermes_binary());
    // Hermes exposes model selection as a global CLI option. It must appear
    ...
    if let Some(permission) = permission_config(permission_mode) {
        command.env("HERMES_PERMISSION", permission.to_string());
    }
    command
}
...
mod tests {
    ...
    #[test]
    fn inherit_preserves_hermes_configuration() {
