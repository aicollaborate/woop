...
// The launcher bundle only needs the @earendil-works/pi-ai portion of the vendored pnpm store,
// but runtime.ensure later spawns a sub-runtime that imports workspace packages such as
// @deepseek-ai/dsh-app-boot. Those live in vendor/.pnpm and are only materialised when pnpm
// install has linked the workspace members into vendor/node_modules/@deepseek-ai. Skip the
// install when those links already exist so incremental tauri dev rebuilds stay fast.
const vendorNodeModules = resolve(vendor, 'node_modules/@deepseek-ai')
const vendorReady = existsSync(resolve(vendorNodeModules, 'dsh-app-boot'))
if (!vendorReady) {
...
// harness sources: `npm --prefix app/flowix-dsh-host run build:runtime`.
