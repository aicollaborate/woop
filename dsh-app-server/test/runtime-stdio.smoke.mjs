...
try {
  ...
  check('initialize returns serverInfo + capabilities',
    init.result?.serverInfo?.name === 'dsh-app-server' && init.result?.capabilities?.approvals?.decisions?.length > 0,
  ...
  check('fresh host reinitializes', reinit.result?.serverInfo?.name === 'dsh-app-server')
